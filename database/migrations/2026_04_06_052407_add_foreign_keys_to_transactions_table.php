...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('transactions', function (Blueprint $table) {
            $table->foreign(['related_event_id'], 'fk_related_event')->references(['id'])->on('events')->onUpdate('restrict')->onDelete('set null');
            $table->foreign(['related_event_location_id'], 'fk_related_event_location')->references(['id'])->on('event_locations')->onUpdate('restrict')->onDelete('set null');
    ...
    {
        Schema::table('transactions', function (Blueprint $table) {
            $table->dropForeign('fk_related_event');
            $table->dropForeign('fk_related_event_location');
